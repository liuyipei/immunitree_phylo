...
%% current j
    ...
    a_tree_children = zeros(1, length(a_tree_parents));
    for node = 1:length(a_tree_parents)
        ...
        if parent == 0            
            ...
            a_tree_children(parent) = a_tree_children(parent) + 1;
        end
    end    
    
    [widest_depth, widest_depth_width] = mode(a_tree_depths);
    % recompute widest_depth_width to account for children
    widest_depth_width = sum(max(1, a_tree_children(a_tree_depths == widest_depth)));     
    ...
    % determine labels, for differential coloring
    
    assert(length(a.t) == length(X), 'a.t does not have the same length as X!')
    %timestamp_strings = {'day06', 'day07', 'day08', 'day09', 'day11', 'day13'};
    %timestamp_strings = {'|day00', '|day07', '|day21', '|flu-binding', '|non-flu', '|unknown'};
    timestamp_strings = {'|day00.454', '|day07.454', '|day21.454', 'flu-binding', 'non-flu'};
    per_read_labels = 6 * ones(length(X), 1); % default value is 6 (dark dark red for 'unknown')
    for vi=1:size(X,1)
        for vj = 1:length(timestamp_strings)                    
            if regexp(X(vi).Header, timestamp_strings{vj}),
