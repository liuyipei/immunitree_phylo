% for plos pathogen submission with sok et al
% by reviewer request, we needed a tree without error correction
...
%-%% %%%%%%%%%%%%%%%%%        Build Tree     %%%%%%%%%%%%%%%%%%%%
    ...
    %priors.NT = generate_sticky_with_indel_prior(2e-1, 5, 6.3e2, 0.05);
    %priors.R = generate_sticky_with_indel_prior(1e-100, 5, 6.3e6, 0.65); % was 6.3e6
    priors.nClasses = 1;
    ...
    %init.mut_model.NT = generate_sticky_with_indel_prior(2e-1, 5, 1, 0.05);
    %init.R = generate_sticky_with_indel_prior(1e-100, 5, 1, 0.65);
    ...
    %% for reviewers: disable read error correction -- make a new child node for each unique sequence
    sef_tree = best.tree; %sequence-error-free tree
    sef_t = best.t;
    sef_sequences = best.sequences;
    num_sef_nodes = size(sef_tree,1);
    for curr_node = 1:size(best.tree, 1)
        curr_concensus = best.sequences(curr_node, :);
        curr_uniqs = unique(uniq_from_raw(best.t==curr_node))'        
        for u = curr_uniqs
            %u            
            if any(Vt_clone_diverse(raw_from_uniq(u), :) ~= curr_concensus)
                
                num_sef_nodes = num_sef_nodes + 1;                
                sef_tree(num_sef_nodes, :) = best.tree(curr_node, :);
                % split off as sibling or child?
                % sef_tree(num_sef_nodes, 1) = curr_node;                 
                sef_t(uniq_from_raw == u) = num_sef_nodes;
                sef_sequences(num_sef_nodes, :) = Vt_clone_diverse(raw_from_uniq(u), :);
            end
        end
    end
    best.tree = sef_tree;
    best.t = sef_t;
    best.sequences = sef_sequences;
    %%
